...
size: .word  13             # size of "array" 
      ...
      sw   $t2, 8($t0)      # F[2] = 1 also
      addi $t1, $t5, -2     # Counter for loop, will execute (size-2) times
loop: lw   $t3, 0($t0)      # Get value from array F[n] 
      lw   $t4, 4($t0)      # Get value from array F[n+1]
      lw   $t6, 8($t0)      # Get value from array F[n+2]
      add  $t2, $t3, $t4    # partial addition F[n] + F[n+1]
      add  $t2, $t2, $t6    # $t2 = $t2 + F[n+2]
      sw   $t2, 12($t0)     # Store F[n+3] = F[n] + F[n+1] + F[n+2] in array
...
head: .asciiz  "The 'Tri'-bonacci numbers are:\n"
      .text
